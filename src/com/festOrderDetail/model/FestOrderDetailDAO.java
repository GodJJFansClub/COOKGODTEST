...
public class FestOrderDetailDAO implements FestOrderDetail_Interface {
	...
	//請不要隨意更動
	private static final String INSERTWITHOR = "INSERT INTO FEST_ORDER_DETAIL (FEST_OR_ID,FEST_M_ID,FEST_OR_QTY,FEST_OR_STOTAL) VALUES (?,?,?,?)";
...
//				System.out.println("DAO" + "224");
	...
	public void insert2(FestOrderDetailVO festOrderDetailVO, Connection con) {
		PreparedStatement pstmt = null;
		try {
			pstmt = con.prepareStatement(INSERTWITHOR);
			// 請不要隨意改動
			pstmt.setString(1, festOrderDetailVO.getFest_or_ID());
			pstmt.setString(2, festOrderDetailVO.getFest_m_ID());
			...
			if (con != null) {
				try {
					...
					System.err.println("rolled back-由--festOrderDetail");
					con.rollback();
