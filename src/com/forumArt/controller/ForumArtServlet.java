	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) 
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req.getRequestDispatcher("/forumArt/select_page.jsp");
			...
			String url = "/forumArt/listOneForumArt.jsp";
...
//			}
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
			req.setAttribute("forumArtVO", forumArtVO); // 資料庫取出的empVO物件,存入req
			String url = "/forumArt/update_forumArt_input.jsp";
...
//			}
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			System.out.println(forum_art_startString);
			java.sql.Timestamp forum_art_start = null;
			try {
				forum_art_start = new java.sql.Timestamp(df.parse(forum_art_startString).getTime());
				System.out.println(forum_art_start);
			} catch (ParseException e) {
				e.printStackTrace();
			}
			//文章狀態
			
			String forum_art_status = req.getParameter("forum_art_status").trim();
			if (forum_art_status == null || forum_art_status.trim().length() == 0) {
				errorMsgs.add("文章狀態.請勿空白");
			}	
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/forumArt/update_forumArt_input.jsp");
				failureView.forward(req, res);
				return; // 程式中斷
			}
...
//			}
		...
		if ("delete".equals(action)) { // 來自listAllEmp.jsp
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/forumArt/listAllForumArt.jsp");
				failureView.forward(req, res);
			}
		}
	}
